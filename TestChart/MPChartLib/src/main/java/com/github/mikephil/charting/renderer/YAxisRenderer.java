    ...
    protected void drawYLabels(Canvas c, float fixedPosition, float[] positions, float offset) {
        ...
        if (mDrawReverse) {
            ...
            for (int i = from; i < to; i ++) {
                ...
                mAxisLabelPaint.getTextBounds(text, 0, text.length(), mDrawTextRectBuffer);
                //Log.d("Kang", "rect : " + mDrawTextRectBuffer.toString());
                if (mDrawTextRectBuffer.left > rectleft) rectleft = mDrawTextRectBuffer.left;
                if (mDrawTextRectBuffer.width() > rectwidth) rectwidth = mDrawTextRectBuffer.width();
            }
            // draw
            for (int i = from; i < to; i++) {

                drawOffsetX = 0.f + Utils.convertDpToPixel(5f);
                drawOffsetY = 0.f;
                ...
                drawOffsetX += mDrawTextRectBuffer.width() * 0.2f;
                drawOffsetY -= lineHeight * 0.7f;

                c.save();
                c.translate(fixedPosition, positions[i * 2 + 1] + offset);

                c.scale(-1, 1);

//            Log.d("Kang", "fixedPosition : " + fixedPosition+ ", positions[i * 2 + 1] + offset : " + (positions[i * 2 + 1] + offset) );
//            Log.d("Kang", "drawOffsetX : " + drawOffsetX+ ", drawOffsetY : " + (drawOffsetY) );
                c.drawText(text, drawOffsetX, drawOffsetY, mAxisLabelPaint);
                if (i == to - 1 && !this.mUnit.equals("")) c.drawText(this.mUnit, drawOffsetX+3, drawOffsetY-Utils.convertDpToPixel(15f), mAxisLabelPaint);
                c.restore();
            }
        } else {
            // draw
    ...
    public void setDrawReverse(boolean b){
        mDrawReverse = b;
    }
