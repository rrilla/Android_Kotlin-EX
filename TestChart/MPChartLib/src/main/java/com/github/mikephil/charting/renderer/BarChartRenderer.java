    ...
    protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
        ...
        for (int j = 0; j < buffer.size(); j += 4) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                c.save();
                c.clipRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                        buffer.buffer[j + 3]);
                Rect rect = new Rect();
                c.drawRoundRect(buffer.buffer[j], buffer.buffer[j + 1], buffer.buffer[j + 2],
                        buffer.buffer[j + 3], buffer.buffer[j + 2]/2, buffer.buffer[j + 2]/2, mRenderPaint);
                c.restore();
                c.drawRect(buffer.buffer[j], buffer.buffer[j + 1] + (buffer.buffer[j + 3] - buffer.buffer[j + 1]) / 2, buffer.buffer[j + 2],
